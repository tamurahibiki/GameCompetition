...
namespace
{
	Vector3 MAP_CENTER_POSITION = Vector3(740.0f, 340.0f, 0.0f);
...
{
	//マップの黒い画像。
	m_spriteRender.Init("Assets/sprite/map.DDS", 400, 400);
...
void Map::Update()
{
	...
	Vector3 enemyPositions = m_enemy->GetPosition();

	Vector3 mapPosition;

	//マップに表示する範囲に敵がいたら。
	if (WorldPositionConvertToMapPosition(playerPosition, enemyPositions, mapPosition))
	{
		//マップに表示するように設定する、
		m_isImage = true;
		//SpriteRenderに座標を設定。
